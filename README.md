...
- Electron 38 + React 19 + TypeScript（严格模式）
...
- pnpm 10 会忽略原生依赖的构建脚本，需要手动补：
  - Electron 二进制：`cd node_modules/.pnpm/electron@*/node_modules/electron && ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ node install.js`
  - better-sqlite3 双 ABI：`npx prebuild-install -r electron -t 38.8.6`（运行时）+ `-r node -t <node版本>`（开发期脚本）
